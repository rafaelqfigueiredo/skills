...
Framework-aware feature planning — stress-tests the goal, detects the stack, then produces a PRD and backlog stored in `.plans/`.

## Usage

```
/fig:plan             # Plan the current task or feature
/fig:plan <goal>      # Plan a specific feature or change
```

## Step 1 — Grill the goal

Read and follow [`skills/fig/grill-me/SKILL.md`](../skills/fig/grill-me/SKILL.md). Ask probing questions until every ambiguity is resolved:
...
Read and follow [`skills/fig/detect-stack/SKILL.md`](../skills/fig/detect-stack/SKILL.md). Load the matching framework skill now — its conventions inform the PRD, backlog, and plan.
...
Synthesize the grilling session into a structured PRD. Read and follow [`skills/fig/to-prd/SKILL.md`](../skills/fig/to-prd/SKILL.md). Apply framework conventions from step 2 when describing the solution and implementation approach.

Save the PRD to `.plans/NNN-slug/prd.md`. Present it to the user and wait for confirmation before proceeding.

## Step 5 — Backlog

Decompose the PRD into backlog tickets. Read and follow [`skills/fig/to-backlog/SKILL.md`](../skills/fig/to-backlog/SKILL.md). Use framework-specific terminology and patterns from step 2 when naming and describing tickets.
